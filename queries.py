...
def select(service):
    result = cur.execute("SELECT * FROM Password_Manager WHERE Service = ?",(service.lower(),)).fetchall()
    if not result:
        print("Not found")
...
def add(data):
    if cur.execute("SELECT * FROM Password_Manager WHERE Service = ?",(data[0].lower(),)).fetchall():
        print("Already there!")
        return
    
    enc_data =  [ (data[0].lower(),enc(data[1]),enc(data[2])) ]
    cur.executemany("INSERT INTO Password_Manager VALUES (?,?,?)",enc_data)
    content.commit()


def Check_before_Edit(con):
    if not cur.execute("SELECT 1 FROM Password_Manager WHERE Service = ?", (con.lower(),)).fetchone():
        return 1
    else:
        return 0
...
def edit(con,data):
    ...
    if con == "main":
        ...
        cur.execute(
            "UPDATE Password_Manager SET Service = ?, Email = ?, Password = ? WHERE Service = ?",
            ("main","NULL", hashed_password, "main"),
        )
        content.commit()

    else:
        enc_data =  [ (data[0].lower(),enc(data[1]),enc(data[2]),con) ]
